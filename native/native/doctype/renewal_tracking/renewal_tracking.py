...
class RenewalTracking(Document):
    ...
    def calculate_item_values(self):
        """Calculate amount, base_rate and base_amount for each item"""
        # Get exchange rate
    ...
    def validate_license_dates(self):
        ...
        if not self.license_start or not self.license_end:
            return  # reqd field handles mandatory enforcement on submit
...
def get_items_from_sales_order(sales_order):
    ...
    for item in so_doc.items:
        item_dict = {
            ...
            'oum': item.uom,  # map SO's uom → our oum field
            'qty': item.qty,
            'rate': item.rate,
        }
    ...
    # Return items along with currency and exchange rate from Sales Order
    return {
        'items': items,
        'currency': so_doc.currency,
        'exchange_rate': so_doc.conversion_rate if hasattr(so_doc, 'conversion_rate') else 1.0
    }
